    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
        ...
        <div className="anim-fade-up" style={{
          ...
          <div style={{ color: C.muted, fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.09em', marginBottom: 20 }}>
            Modelo de ROI — Passo a Passo
          </div>
          {[
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 18 }}>
            <div>
              ...
              <div style={{ fontSize: 11, color: C.muted, marginTop: 3 }}>
                Economia líquida: {fmtBRL(economia)}
              </div>
            </div>
        ...
        <div className="anim-fade-up" style={{
          ...
          <ResponsiveContainer width="100%" height={260}>
            <BarChart data={compData} margin={{ top: 4, right: 4, bottom: 4, left: 16 }}>
              ...
              <YAxis tick={{ fontSize: 10, fill: C.muted }} tickFormatter={(v: number) => `${(v/1000).toFixed(0)}k`} />
      ...
      <div className="anim-fade-up" style={{
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          <div style={{ color: C.dim, fontSize: 12, marginTop: 5 }}>
            ~20% dos beneficiários concentram{' '}
            <span style={{ ...MONO, color: C.blue }}>{at20.toFixed(0)}%</span>
            {' '}do custo projetado — argumento direto para priorização de intervenção
          </div>
        </div>
        <ResponsiveContainer width="100%" height={280}>
          <ComposedChart data={pareto} margin={{ top: 4, right: 20, bottom: 24, left: -10 }}>
            ...
            <XAxis dataKey="pb" tick={{ fontSize: 10, fill: C.muted }}
              label={{ value: '% Beneficiários', position: 'insideBottom', offset: -14, fill: C.muted, fontSize: 11 }} />
            <YAxis tick={{ fontSize: 10, fill: C.muted }}
              label={{ value: '% Custo Acumulado', angle: -90, position: 'insideLeft', fill: C.muted, fontSize: 11 }} />
            <Tooltip content={<ChartTip />} />
      ...
      <div className="anim-fade-up" style={{
        ...
        {[
          ...
          { label: 'Custo com intervenção', val: fmtBRL(comInterv),   color: C.med  },
          { label: 'Economia potencial',    val: fmtBRL(economia),    color: C.low  },
        ].map(s => (
          <div key={s.label} style={{
            background: C.surface, border: `1px solid ${C.border}`, borderRadius: 12, padding: '20px 24px',
          }}>
            <div style={{ color: C.muted, fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.09em', marginBottom: 10 }}>{s.label}</div>
            <div style={{ ...MONO, fontSize: 22, color: s.color }}>{s.val}</div>
          </div>
        ))}
      </div>
    </div>
